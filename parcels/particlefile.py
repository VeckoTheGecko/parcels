...
class ParticleFile:
    ...
    def _create_variables_attribute_dict(self):
        ...
        attrs = {
            ...
            "trajectory": {
                ...
                "_FillValue": _DATATYPE_TO_FILL_VALUE[np.int64],
            },
        ...
        for vname in self.vars_to_write:
            if vname not in ["time", "lat", "lon", "depth", "id"]:
                attrs[vname] = {
                    "_FillValue": _DATATYPE_TO_FILL_VALUE[self.vars_to_write[vname]],
                    "long_name": "",
                    "standard_name": vname,
                    "units": "unknown",
                }
    ...
    def _extend_zarr_dims(self, Z, store, dtype, axis):
        if axis == 1:
            a = np.full((Z.shape[0], self.chunks[1]), _DATATYPE_TO_FILL_VALUE[dtype], dtype=dtype)
            ...
            if len(Z.shape) == 2:
                a = np.full((extra_trajs, Z.shape[1]), _DATATYPE_TO_FILL_VALUE[dtype], dtype=dtype)
            else:
                a = np.full((extra_trajs,), _DATATYPE_TO_FILL_VALUE[dtype], dtype=dtype)
            ...
            for var in self.vars_to_write:
                ...
                if varout not in ["trajectory"]:  # because 'trajectory' is written as coordinate
                    if self._write_once(var):
                        data = np.full(
                            (arrsize[0],),
                            _DATATYPE_TO_FILL_VALUE[self.vars_to_write[var]],
                        ...
                        data = np.full(
                            arrsize, _DATATYPE_TO_FILL_VALUE[self.vars_to_write[var]], dtype=self.vars_to_write[var]
                        )
